...
可以在Message的Headers中设置**x-expires**来控制消息的过期时间

也可以在Queue的args中设置**x-message-ttl**来控制该队列中所有消息的过期时间

如果两个同时设置了，取最小值，但RabbitMQ只会对队列头部的消息进行过期淘汰。如果单独给消息设置TTL，先入队列的消息过期时间如果设置比较长，后入队列的设置时间比较短。会造成消息不会及时地过期淘汰，导致消息的堆积。所以最好是相同过期时间的放在同一队列
...
Message message = new Message(("ttl类型消息设置过期时间10s"+now2).getBytes(), messageProperties);
rabbitTemplate.send("directExchange", "ttl", message);
```

过期的消息会发送到死信交换机，其方式可以参考死信交换机，可应用于延时消息。
